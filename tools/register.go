...
// toolDef carries one tool's metadata and how to register it on a server.
type toolDef struct {
	Name        string
	Description string
...
// defineTool builds a toolDef from a typed handler.
func defineTool[In, Out any](
	name, description string,
	h mcp.ToolHandlerFor[In, Out],
) toolDef {
	t := toolDef{Name: name, Description: description}
	t.register = func(s *mcp.Server) {
		mcp.AddTool(s, &mcp.Tool{Name: t.Name, Description: t.Description}, h)
	}
	return t
}
...
// RegisterTools registers every tool in the registry on the server.
